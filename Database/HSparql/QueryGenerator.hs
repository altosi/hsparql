module Database.HSparql.QueryGenerator
  ( createSelectQuery, createConstructQuery, createAskQuery, createUpdateQuery, createDescribeQuery, prefix, var, embeddedTriple, Database.HSparql.QueryGenerator.triple, triple_, mkPredicateObject, constructTriple, constructTriple_, askTriple, askTriple_, updateTriple, updateTriple_, describeIRI, describeIRI_, optional, optional_, service, service_, union, union_, filterExpr, filterExpr_, filterExists, filterExists_, filterNotExists, filterNotExists_, bind, bind_, subQuery, subQuery_, select, selectVars, insertData, as, a, (.//.), (.|.), (..|..), inv, (*.), (+.), (?.), neg, inv', distinct, distinct_, reduced, reduced_, limit, limit_, groupBy, groupBy_, orderNext, orderNextAsc, orderNextDesc, (.:.), iriRef, (.+.), (.-.), (.*.), (./.), (.&&.), (.||.), (.==.), (.!=.), (.<.), (.>.), (.<=.), (.>=.), notExpr, count, sum_, min_, max_, avg, groupConcat, str, lang, langMatches, datatype, bound, sameTerm, isIRI, isURI, isBlank, isLiteral, regex, regexOpts, strlen, substr, ucase, lcase, strstarts, strends, contains, strbefore, strafter, abs_, round_, ceil, floor_, concat_, replace, rand, qshow, Query, Prefix, Variable, VarOrNode (..), BlankNodePattern, Pattern, SelectQuery (..), SelectExpr (..), ConstructQuery (..), AskQuery (..), UpdateQuery (..), DescribeQuery (..), TermLike (..), SubjectTermLike, PredicateTermLike, ObjectTermLike ) where
...
data IRIRef = AbsoluteIRI RDF.Node | PrefixedName Prefix T.Text
  deriving (Show)
...
data GraphTerm = IRIRefTerm IRIRef | RDFLiteralTerm RDF.LValue | NumericLiteralTerm Integer | BooleanLiteralTerm Bool | BNode (Maybe T.Text) | PropertyPathTerm PropertyPathExpr
  deriving (Show)
data VarOrTerm = Var Variable | Term GraphTerm | EmbeddedTriple' VarOrTerm VarOrTerm VarOrTerm | BlankNodePattern' BlankNodePattern
  deriving (Show)
data VarOrNode = Var' Variable | RDFNode RDF.Node
  deriving (Show)
...
data Relation = Equal | NotEqual | LessThan | GreaterThan | LessThanOrEqual | GreaterThanOrEqual
  deriving (Show)
data Function = CountFunc | SumFunc | MinFunc | MaxFunc | AvgFunc | StrFunc | LangFunc | LangMatchesFunc | DataTypeFunc | BoundFunc | SameTermFunc | IsIRIFunc | IsURIFunc | IsBlankFunc | IsLiteralFunc | RegexFunc | StrLenFunc | SubStrFunc | UcaseFunc | LcaseFunc | StrStartsFunc | StrEndsFunc | ContainsFunc | StrBeforeFunc | StrAfterFunc | ConcatFunc | ReplaceFunc | AbsFunc | RoundFunc | CeilFunc | FloorFunc | RandFunc
  deriving (Show)
data ParameterizedFunction = GroupConcat deriving (Show)
data Expr = OrExpr [Expr] | AndExpr [Expr] | NegatedExpr Expr | RelationalExpr Relation Expr Expr | NumericExpr NumericExpr | BuiltinCall Function [Expr] | VarOrTermExpr VarOrTerm | ParameterizedCall ParameterizedFunction [Expr] [(String, String)]
  deriving (Show)
data SelectExpr = SelectExpr Expr Variable | SelectVar Variable
  deriving (Show)
as :: Expr -> Variable -> SelectExpr
e `as` v = SelectExpr e v
data Pattern = QTriple VarOrTerm VarOrTerm VarOrTerm | Filter Expr | FilterExistsPattern GroupGraphPattern | FilterNotExistsPattern GroupGraphPattern | Bind Expr Variable | OptionalGraphPattern GroupGraphPattern | ServiceGraphPattern IRIRef GroupGraphPattern | UnionGraphPattern GroupGraphPattern GroupGraphPattern | SubQuery QueryData
  deriving (Show)
data GroupGraphPattern = GroupGraphPattern [Pattern]
  deriving (Show)
newtype GroupBy = GroupBy Expr
  deriving (Show)
data OrderBy = Asc Expr | Desc Expr
  deriving (Show)
appendPattern :: Pattern -> GroupGraphPattern -> GroupGraphPattern
appendPattern p (GroupGraphPattern ps) = GroupGraphPattern (p : ps)
appendTriple :: a -> [a] -> [a]
...
data PropertyPathExpr = PathLengthOneExpr PathLengthOne | SequencePathExpr PropertyPathExpr PropertyPathExpr | InversePathExpr PropertyPathExpr | AlternativePathExpr PropertyPathExpr PropertyPathExpr | ZeroOrMorePathExpr PropertyPathExpr | OneOrMorePathExpr PropertyPathExpr | ZeroOrOnePathExpr PropertyPathExpr | NegativePropertySetExpr NegativePropertySet
  deriving (Show)
data NegativePropertySet = NegativePropertySetPath PathLengthOne NegativePropertySet | NegativePropertySetPathOne PathLengthOne
  deriving (Show)
...
data PathLengthOne = PathLengthOneIRI IRIRef | PathLengthOneA | PathLengthOneInverse PathLengthOne
  deriving (Show)
